...
router.post('/register', authLimiter, async (req, res) => {
  try {
    ...
    const user  = await User.create({ name: name.trim(), phone: normalizedPhone, password });
    const token = signToken(user._id);
    res.status(201).json({ success: true, token, user: { id: user._id, name: user.name, phone: user.phone } });
  } catch (err) {
    if (err.code === 11000) return res.status(400).json({ success: false, message: 'Ce numéro est déjà utilisé.' });
...
router.get('/me', protect, async (req, res) => {
  res.json({ success: true, user: { id: req.user._id, name: req.user.name, phone: req.user.phone } });
});
