...
// GET /api/data  — récupérer jours, objectifs ET notes
router.get('/', async (req, res) => {
  try {
    const user = await User.findById(req.user._id).select('days goals notes');
    res.json({ success: true, data: { days: user.days, goals: user.goals, notes: user.notes || [] } });
  } catch (err) {
...
// POST /api/data/notes  — créer une note
...
// PATCH /api/data/notes/:noteId  — modifier une note
...
// DELETE /api/data/notes/:noteId
router.delete('/notes/:noteId', async (req, res) => {
  try {
    const { noteId } = req.params;
    await User.findByIdAndUpdate(req.user._id, { $pull: { notes: { id: noteId } } });
    res.json({ success: true });
  } catch (err) {
    res.status(500).json({ success: false, message: 'Erreur serveur.' });
  }
...
// PATCH /api/data/notes/:noteId/pin  — épingler/désépingler
